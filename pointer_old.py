...
def linecontours(cp_info, path):
    ...
    for xx in new_cntset:
        ...
        output = cv2.fitLine(xx, 2, 0, 0.01, 0.01)
        k = output[1] / output[0]
        k = round(k[0], 2)
...
def countpoint(pointlist, path):
    ...
    x_index = int(len(x)/2)
    y_index = int(len(y) / 2)
    cc = (x[x_index], y[y_index])
    cv2.circle(img, cc, 5, (0, 0, 250), 3)
    cv2.imshow(pname + '_center_point', img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    cv2.imwrite(pname + '_center_point' + ptype, img)
    print("center point {}".format(cc))
    return cc
...
def GetAngle(path, outpath, opoint):
    ...
    kb, r, mask = linecontours(ninfo, path) # mask 刻度线的mask
